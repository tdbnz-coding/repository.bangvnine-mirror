...
class JetLink:
    def __init__(self, address, name=None, resolveurl=False, links=False):
...
class JetItem:
    def __init__(self, name, links=None, icon=None, params=None):
        self.name = name
        self.links = links or []
        self.icon = icon
...
class JetExtractorProgress:
    ...
    def update(self, name): return False

class JetExtractor:
    user_agent = "Mozilla/5.0"
    timeout = 10

    def progress_init(self, progress, items): return False
    def progress_update(self, progress, name): return False
